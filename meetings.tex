% Supervisor meeting log -- standalone, NOT \input by thesis.tex.
% Build: tectonic meetings.tex        Newest entry at the top.
...
% Theme: \darkmodetrue for screen, \darkmodefalse for printing.
% The two tag palettes are separate -- print colours go muddy on a dark page.
\newif\ifdarkmode
\darkmodetrue
...
\ifdarkmode
	...
	\definecolor{askc}{HTML}{6FA8FF}   % question to put to him
	\definecolor{raisec}{HTML}{C08CF0} % point I intend to make
	\definecolor{outc}{HTML}{5FD08A}   % what came back
	\definecolor{actc}{HTML}{F0A254}   % action on me
	\definecolor{answerc}{HTML}{8FA3B0}% the answer slot under a question
	\definecolor{rulec}{HTML}{5A6069}
\else
	\definecolor{pagebg}{HTML}{FFFFFF}
	\definecolor{ink}{HTML}{000000}
	\definecolor{askc}{HTML}{1F4E9C}
	\definecolor{raisec}{HTML}{6B3FA0}
	\definecolor{outc}{HTML}{1B6B3A}
	\definecolor{actc}{HTML}{B4530A}
	\definecolor{answerc}{HTML}{6B7A85}
...
\newcommand{\anshead}[1]{\par\vspace{0.7ex}\noindent{\mtag{answerc}{ANSWER}}~#1\par}
\newcommand{\closeask}{%
	\ifaskopen
		\askopenfalse
		\ifx\pendingans\empty
			\anshead{{\color{answerc}\itshape not yet answered}}%
		\else
			\anshead{\pendingans}%
		\fi
	\fi}
...
\begin{itemize}[leftmargin=1.2em, itemsep=1.0ex, parsep=0pt]
	...
	\item \textbf{CRLB (or otherwise) for optimal SiPM placement.}
		Readout gains happen when a SIPM is able to service a larger volume of scintillator; as JPET and light sharing modules fundamtnally trade.
...
\begin{agenda}{Being tactical with PhD direction. Need to constrain the search space; software-bound contributions, grant positioning, contribution classes (characterisations vs novelities), and relation to prior program.}
	...
	\raisept{Discouraging search so far; many negative results (napkin code/calcs) and occupied novelties and non-obvoius gaps - perhaps expected.}
		...
		Converged many times with fields wisdom; Inorganics are simply very good; the case for nanocomposites is manufacturability, not necessarily performance, and scattering and temporal blurring still hurt composites and glass ceramics.
	...
	\raisept{Thesis directions positioned for on top-up opportunities; pressumably ANSTO}
		AINSE PGRA 2027 opens $\sim$Feb; condition is facility use \textbf{and/or} an ANSTO collaborator.
		Simulation-only seems feesible there: Dingo's validated Monte Carlo model was extended into a 3D Geant4 beamline twin underpinning a new in vitro neutron capture capability (B-10 in glioma cells, Dec 2025) --- squarely Safavi-Naeini's scope.
	...
	\ask{I think the monolithic PET moves forward defensible only if the manufacturability of nanocomposites is meaningfully exploited - conventional monolithic geometries are already well served. How, if at all, do you see the monolithic program likely progressing, and what does it need?}
		...
		Information-theoretic optimsation of SIPM placement could be justified, but CRLB was applied to monolithic PET around 2012, so the claim would be the design-optimisation use, not the bound itself.
		Alternatively, the program before me investigated the recoverability of interaction localisation, and recently multi-interaction events, but perhaps not monolithic nanocomposite effects on temporal resolution (Rayleigh, Compton, and Mie scattering in larger-particle composites add stochasticity to the photon transport to SIPMs, especialy with anistoropy and imperfections) and energy resolution (optical photons must be attributed to a particular interaction - overlapping flashes noise eachother).  
	...
	\ask{Is 'monolithic PET', 'image reconstruction', and 'experimental validation' in the proposal title binding?}
	...
	\ask{How should I prioritise in my thesis direction? Space for publishability? Capitalise on leverages we have (simulation, ML, collaborators?)}
