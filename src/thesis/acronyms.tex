% Acronyms and Terms -- maintained BY HAND, sorted AUTOMATICALLY. In the body
% just type the acronym or term as plain text (PET, CT, ...); nothing expands or
% links. Two entry types, both sorting into the same alphabetical list:
%
...
%
% An acronym bolds its expansion inside the meaning column, then glosses it
% after an em-dash; a term has nothing to expand, so its meaning column is the
% gloss alone. Order does not matter -- the list is sorted case-insensitively
% when printed (expl3, no packages -- datatool was cut because its load-and-sort
% cost helped push Overleaf past its compile timeout).
% Expandable engine string compare: \pdfstrcmp on pdfTeX (Overleaf),
% \strcmp on XeTeX (tectonic).
\ifdefined\pdfstrcmp\let\acrstrcmp\pdfstrcmp\else\let\acrstrcmp\strcmp\fi
\ExplSyntaxOn
\seq_new:N \g__acr_seq
\bool_new:N \g__acr_first_bool
% Case fold for the sort key; renamed across expl3 releases, so take
% whichever this kernel has (fall back to raw compare on neither).
...
% A term renders identically -- bold key, meaning beside it -- so it is the same
% mechanism. Separate name purely so the source says which kind an entry is.
...
\term{annihilation photons}{the two \keV{511} photons produced when a positron annihilates with an atomic electron. Named to distinguish them from gamma rays, which originate inside the nucleus}
\term{positron range}{the displacement between the point of emission and the point of annihilation. PET reconstructs the latter while the tracer chemistry occurs at the former, so it sets a hard resolution limit no detector design can recover}
...
% Column 1 is a fixed-width p, not an l: an acronym is always short, but a
% multi-word \term would stretch an l column and crush the meaning beside it.
