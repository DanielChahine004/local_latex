...
%
% QUALITATIVE: band widths are schematic (the trunk halves at each interface).
% The claims live in the ledger, whose ranges are order-of-magnitude and still
% want a source, and which must keep body scatter and solid angle dominant --
% of those, only solid angle is a design choice.
% SCATTER destroys the LOR, not the photon, so the trunk tracks only photons
% that keep a valid LOR.
...
%% node text: nested in a group it breaks TikZ's line breaker, hence the bare
%% font switch.
...
%% Terminal losses, named beside their node: the ribbons are too narrow.
...
%% Tightest spacing in the figure: three two-line labels sharing the top 2.6 cm
%% with 0.2 cm of white between. The crystal leader must stay above the
%% solid-angle label (leader y -1.63 vs label top -2.27 near x 7.05).
...
%% note is a separate node: prose in a p-cell halts TeX ("Missing \endgroup").
