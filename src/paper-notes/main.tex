% Working notes on papers -- standalone, NOT part of the thesis. Analyse a
% paper here as soon as it is read; copy anything the thesis needs by hand.
% Build: pixi run build-notes.
%
% One entry per paper. Lead with \paragraph{Summary} -- who did what, in
% plain terms -- so the entry says what the paper even did before the
% Capsule argues why it matters. Free-form after that (\paragraph headings
% suit dense papers; plain paragraphs are fine too):
%
%   \begin{papernote}{Author Year -- short descriptive title}{BibKey}
%   ...
%   \end{papernote}
%
% BibKey is the references.bib key (shared with the thesis).
%
% Images live in img/ next to this file and are referenced by relative path.
% \thumb{img/annpet.png} right after \begin{...note} is the
% image that fronts the entry (a small figure here, the card image on the
% canvas); \includegraphics[width=...]{file} anywhere in the body is an inline
% figure in both. \doi{10.1088/...} in a papernote is the paper's link, clickable
% in the PDF and on the canvas card (\href/\url work too).
%
% Structure: a paper that belongs to a programme (J-PET, AnnPET, ...) sits
% inside that programme's programnote -- overview first (actor-level facts:
% membership, detector lineage, IP, scope boundary), then its papers in the
% programme's reading order, nested in the TOC. Any other paper is a
% top-level papernote. The TOC is the index.
...
\usepackage{xurl}
\usepackage{graphicx}
% the image that fronts an entry: small, centred, no caption
\newcommand{\thumb}[1]{\begin{center}\includegraphics[width=0.3\linewidth]{#1}\end{center}}
% where a paper lives: a clickable doi.org link here, the card's link on the canvas
...
% A papernote is a \section, except inside a programnote, where it drops to
% \subsection so the programme's papers nest under it in the TOC.
% Top-level TOC entries carry their kind (Paper: / Programme:), TOC-only;
% nested papers need no tag -- the indentation already says it.
\newcommand{\papernoteheading}{\section}
\newcommand{\papernotetag}{Paper: }
\NewDocumentEnvironment{papernote}{mm}
	{\papernoteheading[\papernotetag#1]{#1}\label{note:#2}}
	{}
\NewDocumentEnvironment{programnote}{mm}
	{\section[Programme: #1]{#1}\label{prog:#2}%
	 \renewcommand{\papernoteheading}{\subsection}%
	 \renewcommand{\papernotetag}{}}
	{}

% Something not yet verified against the source PDF.
% breakable space after the tag, or long gaps overflow narrow lines
\newcommand{\gap}[1]{\textcolor{red}{\textbf{[gap:} #1\textbf{]}}}
...
in their own words. Note this: it is a clean, citable instance of design-by-scaling
...
These system numbers are context only --- they do not belong in any chapter of
...
\begin{itemize}
	\item \textbf{Performance:} \mm{0.78} 2D FWHM in the $(x,y)$ plane,
		...
		\textbf{Caveat: the crystal is \mm{8} thick.} These figures do not
...
monolithic crystals, and the review cites it as such. \gap{Confirm Albira's
...
Note this is a group with an ERC AdG, an NIH R01, a spin-off, and a decade of
...
review in \emph{J Nucl Med} 2022 cited by \ref{note:Xie2026}.
...
Thull's own address is LfB --- he came into PET from hematopoietic cell
...
TRPMS 2024) --- relevant separately for the BGO and Cherenkov work.
...
\begin{itemize}
	\item Espa\~na et al.\ 2014 PMB --- DigiPET, the start of the lineage; its own
	...
	\item The walk-through line has its own notes below: PET20.0
...
cylinder at comparable sensitivity. System detail is held in
...
and is a quick guide when comparing geometries. That is exactly the use the
...
a LuF\textsubscript{3}:Ce--polystyrene nanocomposite. The co-supervisor's own
