% !TeX root = ../thesis.tex
%
% Drafting scaffold: working notes on individual papers, one \section each.
% This is a staging area, not a thesis chapter. Analyse a paper here as soon as
% it is read -- summary, findings, numbers worth quoting -- without deciding
% where it belongs. When a home is found, mine the note into real prose (or
% \ref{note:<key>} it from a \topics block as a reminder) and delete the note.
% Before submission, delete this file and its \input in thesis.tex.
%
% Template -- copy for each paper (use the references.bib key in the label):
%
%   \section{Author Year -- short descriptive title}
%   \label{note:BibKey}
%   \paragraph{Summary}
...
\topics{one section per paper analysed; drop the template fields that do not
    apply; keep concrete numbers (resolution, timing, cost figures) verbatim
    with units, since these are what the real chapters will want to cite.}

\section{Thull et al.\ 2026 -- Uncertainty-aware gamma interaction localisation and reconstruction}
\label{note:T}
